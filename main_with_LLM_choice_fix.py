...
from botorch.acquisition import (
    qLogExpectedImprovement, qProbabilityOfImprovement,
    qUpperConfidenceBound, qKnowledgeGradient, qLogNoisyExpectedImprovement
)
...
class ChatBot:
    ...
    def chat_without_context_update(self, user_input):
        """
        基于当前上下文发送用户输入并获取模型回复，回复不写入默认上下文
        :param user_input: 用户输入的文本
        :return: 模型的回复文本（失败返回错误信息）
        """
        # 复制当前上下文（浅拷贝即可，避免修改原上下文）
        temp_context = self.context.copy()
        
        # 将当前用户输入添加到临时上下文（仅在副本中）
        temp_context.append({
            "role": "user",
            "content": user_input
        })
    ...
    def replace_prompt_from_md5(self, md_file_path, field1_value, field2_value, field3_value, field4_value, field5_value):
        try:
            # 1. 校验文件路径是否存在
            if not os.path.exists(md_file_path):
                raise FileNotFoundError(f"文件不存在：{md_file_path}")
            ...
            # 4. 定义三个特定字段的占位符（可根据需求修改）
            placeholders = {
                "[choice1]": field1_value,
                "[choice2]": field2_value,
                "[choice3]": field3_value,
                "[choice4]": field4_value,
                "[choice5]": field5_value,
            }
...
def read_sp_file_to_string(file_path: str) -> str:
    with open(file_path, 'r', encoding='utf-8') as sp_file:
        # 读取文件全部内容并返回字符串
...
def get_focused_normalized_bounds(bounds_to_norm, ref_bounds):
    """
    修正维度不匹配问题：拆分下界/上界分别标准化
    """
    # 1. 校验两个字典的参数名和顺序一致
    param_names = list(bounds_to_norm.keys())
    if param_names != list(ref_bounds.keys()):
        raise ValueError("两个字典的参数名/顺序不一致！")
    d = len(param_names)  # 参数数量
    
    # 2. 提取参考边界（形状：(2, d)）：第0行=所有参数参考下界，第1行=所有参数参考上界
    ref_lower = torch.tensor([ref_bounds[p][0] for p in param_names], dtype=torch.double)  # (d,)
    ref_upper = torch.tensor([ref_bounds[p][1] for p in param_names], dtype=torch.double)  # (d,)
    ref_bounds_tensor = torch.stack([ref_lower, ref_upper])  # (2, d)
    
    # 3. 提取待标准化的边界：拆分为下界数组和上界数组（形状均为 (1, d)）
    to_norm_lower = torch.tensor([bounds_to_norm[p][0] for p in param_names], dtype=torch.double).reshape(1, d)  # (1, d)
    to_norm_upper = torch.tensor([bounds_to_norm[p][1] for p in param_names], dtype=torch.double).reshape(1, d)  # (1, d)
    
    # 4. 分别标准化下界和上界（关键：维度匹配，(1, d) 和 (2, d) 兼容）
    normalized_lower = normalize(X=to_norm_lower, bounds=ref_bounds_tensor)  # (1, d)
    normalized_upper = normalize(X=to_norm_upper, bounds=ref_bounds_tensor)  # (1, d)
    
    # 5. 拼接成optimize_acqf要求的(2, d)形状
    focused_bounds = torch.cat([normalized_lower, normalized_upper], dim=0)  # (2, d)
    
    return focused_bounds
def extract_choice_number(long_string: str) -> int:
    lines = long_string.split('\n')
    target_line = None
    for line in lines:
        if "Choice:" in line:
            target_line = line.strip()  # 去除首尾空白字符
            break 
    # 如果没找到包含"Choice:"的行，直接返回默认值1
    if not target_line:
        return 1
    bracket_match = re.search(r'\{([^}]+)\}', target_line)
    if not bracket_match:
        return 1
    content_inside_brackets = bracket_match.group(1).strip()
    number_match = re.search(r'\d+', content_inside_brackets)
    if not number_match:
        return 1
    try:
        number = int(number_match.group())
    except ValueError:
        print("无法将提取的内容转换为整数，返回默认值1")
        return 1
    print(f"提取到的Choice编号：{number}")
...
class BayesianOptimizer:
    ...
    def canditate_to_string(self,canditate):
        X=[]
        for idx, candidate in enumerate(canditate):
        # 反归一化：还原到原始参数范围（修复维度问题）
            next_params1 = unnormalize(candidate.unsqueeze(0), self.bounds).squeeze(0)
            params_dict1 = self.params_to_dict(next_params1)
            X.append(next_params1.numpy())
            sample_string = generate_sample_string(X, self.config.PARAM_BOUNDS)
        return sample_string
    
    def run_initial_sampling(self, llm_prompt_md_path=r'original_prompt.md'):
     """
    ...
    # 将numpy数组列表转换为单个二维numpy数组，再转张量
        if iteration ==1:
            ...
            print(f"初始采样点:\n{initial_result}")
            para=extract_param_names_with_ranges_simple(self.Param_Bounds)
            print(f"{para}") 
            llm_prompt = self.llm.replace_prompt_from_md(
            md_file_path=r'inturn_first_prompt.md',
            field1_value=initial_result,
            field2_value=para,
            field3_value=" ")
            llm_response = self.llm.chat(llm_prompt)
            self.Param_Bounds=update_param_bounds(llm_response, self.config.PARAM_BOUNDS)
        else:
            ...
            print(f"{para}")  
            llm_prompt = self.llm.replace_prompt_from_md(
            md_file_path=r'inturn_regular_prompt.md',
            field1_value=initial_result,
            field2_value=para,
            field3_value=str(iteration))
            llm_response = self.llm.chat(llm_prompt)
            self.Param_Bounds=update_param_bounds(llm_response, self.config.PARAM_BOUNDS)
        focused_bounds= get_focused_normalized_bounds(self.Param_Bounds, self.config.PARAM_BOUNDS)
    ...
    # 替换LogExpectedImprovement为qExpectedImprovement（支持q>1）
        acq1 = qLogExpectedImprovement(
        model=gp,
        best_f=train_Y.max(),  # 历史最优值
        )    
    # 优化采集函数：批量采样5个候选点
        candidates1, acq_values = optimize_acqf(
        acq1,
        ...
        c1= self.canditate_to_string(candidates1)
        acq2 = qProbabilityOfImprovement(
        model=gp,
        best_f=train_Y.max(),
        tau=0.01,  # 改进阈值：只计算比最优值好tau以上的概率
        )
        candidates2, acq_values = optimize_acqf(
        acq2,
        bounds=focused_bounds,
        q=5,  # 批量采样5个点
        num_restarts=10,
        ...
        c2= self.canditate_to_string(candidates2)
        acq3 = qKnowledgeGradient(
        model=gp,
        num_fantasies=12,  # 幻想样本数，平衡计算量和精度
        )  
        candidates3, acq_values = optimize_acqf(
        acq3,
        bounds=focused_bounds,
        q=5,  # 批量采样5个点
        num_restarts=10,
        ...
        c3= self.canditate_to_string(candidates3)
        acq4 = qLogNoisyExpectedImprovement(
        model=gp,
        X_baseline=train_X,
        )
        candidates4, acq_values = optimize_acqf(
        acq4,
        bounds=focused_bounds,
        q=5,  # 批量采样5个点
        num_restarts=10,
        ...
        c4= self.canditate_to_string(candidates4)
        acq5 = qUpperConfidenceBound(
        model=gp,
        beta=0.1,
        )
        candidates5, acq_values = optimize_acqf(
        acq5,
        bounds=focused_bounds,
        q=5,  # 批量采样5个点
        num_restarts=10,
        ...
        c5= self.canditate_to_string(candidates5)
        choice_prompt = self.llm.replace_prompt_from_md5(r'inturn_choice_prompt.md',c1,c2,c3,c4,c5)
        llm_choice = self.llm.chat_without_context_update(choice_prompt)
        choice_number = extract_choice_number(llm_choice)
        print(f"LLM选择的采集函数编号：{choice_number}")
        if choice_number == 1:
    ...
    def optimize(self):
        ...
        for i in range(1, self.config.N_ITERATIONS + 1):
            ...
            if self.best_fom == 3.0:
                print("🎉 达到最优FOM，提前终止优化！")
                break
...
def extract_equality_relations(input_str):
    # 初始化空字典存储结果
    equality_dict = {}
    ...
    for line in lines:
        ...
        # 只处理包含"Equality"的非空行
        if stripped_line and 'Equality' in stripped_line:
            # 找到左花括号和右花括号的位置
            left_brace = stripped_line.find('{')
            right_brace = stripped_line.find('}')
            # 确保找到成对的花括号
            if left_brace != -1 and right_brace != -1 and right_brace > left_brace:
                # 提取花括号中间的内容
                content = stripped_line[left_brace + 1 : right_brace].strip()
                # 按等号分割成键和值
                if '=' in content:
                    key, value = content.split('=', 1)  # 只分割第一个等号，避免内容含多个等号
                    equality_dict[key.strip()] = value.strip()
    
    return equality_dict
    # 输出: {'w_diff': 'w_diff1', 'l_diff': 'l_diff1', 'w_load': 'w_load1', 'l_load': 'l_load1'}
def remove_mapped_params(equality_map, param_bounds):
    # 先复制原始字典，避免修改原字典
    processed_bounds = param_bounds.copy()
    # 提取所有需要删除的参数名（即equality_map中的值）
    params_to_remove = set(equality_map.values())
    # 遍历需要删除的参数，逐个移除
    for param in params_to_remove:
        if param in processed_bounds:
            del processed_bounds[param]
    return processed_bounds
def replace_params_in_sp_file(input_path, output_path, replace_map):
    try:
        # 1. 读取原始.sp文件内容
        with open(input_path, 'r', encoding='utf-8') as f:
            file_content = f.read()
        
        # 2. 批量替换参数名：遍历替换字典，将值替换成键
        # 注意：替换顺序不影响（示例中参数名无包含关系，若有需调整顺序）
        for target_param, old_param in replace_map.items():
            file_content = file_content.replace(old_param, target_param)
        
        # 3. 将替换后的内容写入新文件
        with open(output_path, 'w', encoding='utf-8') as f:
            f.write(file_content)
        
        print(f"文件替换完成！已保存至: {output_path}")
    
    except FileNotFoundError:
        print(f"错误：找不到指定的文件 - {input_path}")
    except PermissionError:
        print(f"错误：没有权限读取/写入文件，请检查路径权限")
    except Exception as e:
...
def main():
    ...
    print("🚀 OpAmp 贝叶斯优化系统")
    print("="*70)
    md_file_path=r'equal_prompt.md'
    # 初始化配置
    config = Config()
    print(config.PARAM_BOUNDS)
    LLM=ChatBot(key='',base_url="https://api.deepseek.com/v1",model='deepseek-chat')
    str=LLM.replace_prompt_from_md(md_file_path,' ',' ',' ')
    LLM_response=LLM.chat(str)
    print(LLM_response)
    equal=extract_equality_relations(LLM_response)
    config.PARAM_BOUNDS=remove_mapped_params(equal, config.PARAM_BOUNDS)
    replace_params_in_sp_file(
        input_path=r'opamp.sp',
        output_path=r'opamp1.sp',
        replace_map=equal
    )
    replace_params_in_sp_file(
        input_path=r'opamp copy.sp',
        output_path=r'opamp2text.sp',
        replace_map=equal
    )
